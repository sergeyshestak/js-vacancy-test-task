...
async function validator(ctx: AppKoaContext<AddProductToCart>, next: Next) {
  ...
  ctx.assertClientError(!cart?.cart.includes(productId), {
...
async function handler(ctx: AppKoaContext<AddProductToCart>) {
  ...
  if (cart?.cart) {
    await cartService.updateOne({ userId }, () => ({ cart: [...cart.cart, productId] }));
  } else {
    await cartService.updateOne({ userId }, () => ({ cart: [productId] }));
  }
...
export default (router: AppRouter) => {
  router.post('/add', validateMiddleware(addProductToCartSchema), validator, handler);
};
